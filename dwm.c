...
void scan(void) {
    ...
    if (XQueryTree(dpy, root, &d1, &d2, &wins, &num)) {
        ...
        for (int i = 0; i < num; i++) { /* now the transients */
...
Drw* get_drw_with_fontset(Display *dpy, int screen, Window root, unsigned int w, unsigned int, const char *fonts[], size_t fontcount) {
...
void setup(void) {
    ...
    drw    = get_drw_with_fontset(dpy, screen, root, sw, sh, fonts, LENGTH(fonts));
    lrpad  = drw->fonts->h;
...
int updategeom(void) {
    ...
    /* 		/1* only consider unique geometries as separate screens *1/ */
    ...
    /* 		XFree(info); */
    /* 		nn = j; */
    /* 		if (n <= nn) { /1* new monitors available *1/ */
    /* 			for (i = 0; i < (nn - n); i++) { */
    ...
    /* 					updatebarpos(m); */
    /* 				} */
    /* 		} else { /1* less monitors available nn < n *1/ */
    /* 			for (i = nn; i < n; i++) { */
    /* 				for (m = mons; m && m->next; m = m->next); */
    /* 				while ((c = m->clients)) { */
    /* 					dirty = 1; */
